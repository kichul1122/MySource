...
//http://blog.naver.com/1mi2/220193963897
...
public abstract class StateMachineBehaviourEx : MonoBehaviour
{
    ...
    void ChangingState()
    {
        this.lastState = state.currentState;
        this.lastStateMachineBehaviour = state.executingStateMachine;
    ...
    void ConfigureCurrentState()
    {
        if (state.exitState != null)
        {
            this.exitStateCoroutine.Run(state.exitState());
        }
        ...
        {
            state.enterStateEnumerator = state.enterState();
            this.enterStateCoroutine.Run(state.enterStateEnumerator);
        }
